...
const subServices = [
  {
    ...
    icon: (
      <svg className="w-8 h-8 text-deep-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-charcoal">Core Services</h2>
        ...
        >
          {services.map((service, index) => (
            <motion.div
              ...
              className="bg-white border border-cool-gray/20 rounded-xl p-8 text-center hover:shadow-lg transition-all duration-300"
            >
              <div className="flex justify-center mb-6">
                <div className="bg-ultra-light-gray rounded-full p-4">
                  {service.icon}
                </div>
              </div>
              <h3 className="text-xl font-semibold mb-4 text-charcoal">{service.title}</h3>
              <p className="text-slate-gray mb-6">{service.description}</p>
              <Link href={`/serviceDetails?id=${service.id}`} className="text-deep-teal font-semibold hover:text-opacity-80 inline-flex items-center">
                Learn more
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <h3 className="text-2xl font-bold text-center mb-8 text-charcoal">Additional Features</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {subServices.map((subService, index) => (
              <motion.div
                ...
                className="bg-ultra-light-gray rounded-xl p-6 text-center hover:shadow-md transition-all duration-300"
              >
                ...
                <h4 className="text-lg font-semibold mb-3 text-charcoal">{subService.title}</h4>
                <p className="text-slate-gray text-sm">{subService.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          <Link href="/services" className="bg-deep-teal text-white px-8 py-4 rounded-lg font-semibold hover:bg-opacity-90 transition-all duration-200">
            View All Services
          </Link>
        </motion.div>
      </div>
